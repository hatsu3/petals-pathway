...
do
    ...
    tmux new-session -d -s "Server_${SERVER_ID}" bash ../../src/petals/cli/deploy_server.sh -m "bigscience/test-bloomd" -i ${INITIAL_PEER} -d ${cfg[device]} -p ${cfg[id_path]} -b ${cfg[block_ids]} -a ${cfg[maddr]}
done
